...
| Digital-twin agents | `findAgent` `getAgent` (local composite — `agents.go`, registered outside `allOps()`, drift gate skips them): the people-analog of the skill registry. A user twin (`systemObjType="user"` on the `System` form) whose `description` is an "# Agent" profile is the agent. `findAgent` discovers people by competency over the twins' profiles via `searchAll` (empty query lists members via `getUsers`), `getAgent` loads one twin in full by `userId` (get-or-create via the system-actor endpoint) or `actorId`. Resolves the `System` form id like `skills.go` (cached per workspace). See [`digital-twin-agents.md`](../plugins/simulator/docs/entities/digital-twin-agents.md) and the `/simulator-agents` skill. |
